...
var db = imports.db || leveldb.levelup(config.leveldb + '/blocks', {
...
BlockDb.prototype.getBlockForTx = function(txId, cb) {
  ...
  logger.info('getBlockForTx - Looking up key:', {
    txId: txId,
    key: key
  });
  ...
  db.get(key, function(err, val) {
    if (err && err.notFound) {
      logger.error('getBlockForTx - Key not found:', {
        txId: txId,
        key: key,
        error: 'notFound'
      });
    ...
    if (err) {
      logger.error('getBlockForTx - Error:', {
        txId: txId,
        key: key,
    ...
    // Log the found value
    logger.info('getBlockForTx - Found value:', {
      txId: txId,
      key: key,
      value: val
    });
    ...
    try {
      var v = val.split(':');
      logger.info('getBlockForTx - Parsed result:', {
        txId: txId,
        blockHash: v[0],
        height: parseInt(v[1])
      });
      ...
      logger.error('getBlockForTx - Error parsing value:', {
        txId: txId,
        key: key,
        value: val,
        error: e.message || e
...
// Returns blockHash and height for a given txId (If the tx is on the MAIN chain).
BlockDb.prototype.getBlockForTx2 = function(txId, cb) {
...
BlockDb.prototype._fillConfirmationsOneVin = function(o, chainHeight, cb) {
  var self = this;
  var wasMergeMined = o.isMergeMined;

  self.getBlockForTx(o.txid, function(err, hash, height) {
    if (!hash || height === undefined) {
      // If DB lookup failed, try RPC as fallback
      Rpc.getTxInfo(o.txid, function(rpcErr, txInfo) {
        if (!rpcErr && txInfo && txInfo.confirmations && txInfo.confirmations > 0) {
          o.isConfirmed = true;
          o.confirmations = txInfo.confirmations;
          o.unconfirmedInput = false;
          if (wasMergeMined) {
            o.isMergeMined = wasMergeMined;
          }
          logger.info('Used RPC fallback for transaction confirmation: txid=' + o.txid +
                      ', confirmations=' + o.confirmations);
          return cb();
        } else {
          // Proceed with default behavior if RPC also fails
          o.isConfirmed = false;
          o.confirmations = 0;
          o.unconfirmedInput = true;
          if (wasMergeMined) {
            o.isMergeMined = wasMergeMined;
          }
          return cb();
...
BlockDb.prototype._fillConfirmationsOneVin4 = function(o, chainHeight, cb) {
  var self = this;
  var wasMergeMined = o.isMergeMined;

  logger.info('_fillConfirmationsOneVin - Processing input:', {
    txid: o.txid,
    parentTxid: o.parentTxid || 'unknown', // Add this to track which transaction this is an input for
    wasMergeMined: !!wasMergeMined
  });

  self.getBlockForTx(o.txid, function(err, hash, height) {
    logger.info('_fillConfirmationsOneVin - getBlockForTx result:', {
      txid: o.txid,
      parentTxid: o.parentTxid || 'unknown',
    ...
    if (height >= 0) {
      o.isConfirmed = chainHeight >= height;
      o.confirmations = chainHeight - height + 1;
      logger.info('_fillConfirmationsOneVin - Confirmation calculation:', {
        txid: o.txid,
        height: height,
        chainHeight: chainHeight,
      ...
      logger.info('_fillConfirmationsOneVin - Block height not found for tx:', {
        txid: o.txid,
        height: height
      });
    }
    ...
    logger.info('_fillConfirmationsOneVin - Final input status:', {
      txid: o.txid,
      isConfirmed: o.isConfirmed,
      confirmations: o.confirmations,
      unconfirmedInput: o.unconfirmedInput
    });
...
BlockDb.prototype._fillConfirmationsOneVin3 = function(o, chainHeight, cb) {
  var self = this;
  var wasMergeMined = o.isMergeMined;
  ...
  self.getBlockForTx(o.txid, function(err, hash, height) {
    ...
    if (height >= 0) {
      ...
      // If we have confirmations, ensure unconfirmedInput is false regardless of isConfirmed
...
BlockDb.prototype._fillConfirmationsOneVin2 = function(o, chainHeight, cb) {
  var self = this;
  var wasMergeMined = o.isMergeMined;
