...
var warn   = logger.warn || logger.info;
     ...
     function(c) {
        ...
        if (needReorg) {
          ...
          self.processReorg(oldTip, oldNext, newPrev, oldHeight, function(err, h) {
            if (err) {
              if (err.message && err.message.indexOf('Could not found block') !== -1) {
                warn('Reorg block not found, retrying in 5s...');
                return setTimeout(function() {
                  self.processReorg(oldTip, oldNext, newPrev, oldHeight, function(retryErr, retryH) {
